...
if __name__ == '__main__':
    ...
    dic_to_dump = {k[:-2]: v for k, v in six.iteritems(dic) if k in var_to_dump}
    dic_save = {}
    for key in dic_to_dump.keys():
        print key

    #for key in dic_to_dump.keys():
    #    var = dic_to_dump[key]
    #    layer_name, part_name = key.split('/')
    #    layer_name = unicode(layer_name, 'utf-8')
    #    if layer_name in dic_save:
    #        dic_save[layer_name][part_name] = dic_to_dump[key]
    #    else:
    #        dic_save[layer_name] = dict()
    #        dic_save[layer_name][part_name] = dic_to_dump[key]
    dic_save = dic_to_dump        
    #print dic_save.keys()
    #varmanip.save_chkpt_vars(dic_to_dump, args.output)
    np.save(args.output, dic_save)
